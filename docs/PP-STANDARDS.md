...
- **Card sync (locked 25 Jul 2026; tightened after EP10 review; entry delay superseded above):** every card ENTERS on or just after its spoken cue (never before; timed off the WhisperX-anchored shot map) and HOLDS at least `min_card_hold` (10s from EP09) or until the next card needs the frame. When the cue is a phrase INSIDE the beat (not its opening line), give the card a `"cue"` field in episode.json — QC locates the phrase in the master's SRT and hard-fails a card that enters before the words are spoken. **HARD RULE: b-roll clips, motion cards and the midroll chip NEVER share the screen** — a card writing over a clip means one of them wasn't seen; QC computes every window (assembler maths: `broll_offsets` default +1.0, `broll_dur` 5s) and hard-fails any overlap. **The midroll chip FOLLOWS the start of the spoken ask by +1.0s — it does NOT span it and must never precede it** (Jodie, 29 Jul 2026, SUPERSEDING the previous "must SPAN the spoken ask" rule); anchor `midroll.at` to the SRT. `qc_episode.py --episode` hard-fails a breach of any of these.
  - **Why it changed.** Spanning the ask meant the chip appeared *before* Gordon began it — on EP13 by 0.40s, **which Jodie detected by ear.** The picture was announcing the words instead of following them, which is the same fault as a card entering before its cue.
  - **Why +1.0s and not the +3.0s a card gets.** The 3.0s card delay exists because a normal card illustrates something being **EXPLAINED** — words first, picture after, so the viewer hears the idea before seeing it drawn. **The chip reinforces a direct REQUEST, not an explanation**, and 3.0s is a third of an 8.5s ask. **1.0s follows without lagging.**
  - The **≥6s full-visibility** standard is unchanged.
...
- Article-body HTML uses the template classes: `.kicker`, `h1.section`, `.byline`, `.lead`, `h2.rule`, `blockquote`/`.pullquote`, `.note`, `.divider`, `img.illus` (+ `.portrait`), `.pagebreak`, `.avoid`, `ol > li`, `table.chart`. **The vocabulary is ENFORCED** — an unknown `<p class>` halts, so article prose cannot be dressed up as editorial furniture to slip past the fidelity check. Warranty + marketing pages come from the template, copied byte-identical; a second copy in the body halts.
...
- **A CHART SITS BESIDE THE SECTION IT EXPLAINS — never dumped at the end.** (Jodie, 15 Aug
  ...
  > in the vocabulary. **The only door left open was `omit_paragraphs`, and the writer took
  > it.** The gate refused, correctly — but *a rule that leaves no legal way to do the right
  > thing has already chosen the wrong one.* The SPOKEN script still refers to a chart rather
  > than reading it out, which is right and unchanged: **the video and the book differ here on
  > purpose.**
