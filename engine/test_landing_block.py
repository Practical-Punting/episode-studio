...
class TruncatedDownload(unittest.TestCase):
    ...
    def test_a_short_download_is_refused_and_not_promoted(self):
        ...
        with mock.patch.object(providers.urllib.request, "urlopen",
                               return_value=_Resp(b"x" * 4096, 114_395_315)):
    ...
    def test_the_message_says_a_retry_helps(self):
        """A halt must say plainly whether retrying helps. This one does."""
        with mock.patch.object(providers.urllib.request, "urlopen",
                               return_value=_Resp(b"x" * 10, 100)):
            with self.assertRaises(EngineFlag) as cm:
                RealProvider._download_exact("http://x.test/v.mp4", self.dest)
    ...
    def test_no_stated_length_allows_a_real_file(self):
        ...
        with mock.patch.object(providers.urllib.request, "urlopen",
                               return_value=_Resp(b"z" * 900, None)):
            RealProvider._download_exact("http://x.test/v.mp4", self.dest)
...
class StaleCodeGuard(unittest.TestCase):
    ...
    def test_it_releases_the_lease_before_exiting(self):
        """Exiting while still holding the lease strands the episode — and releasing
        ownership without clearing it is how the DEAD ZONE is reached: a working
        status with claimed_by NULL matches neither claim_next nor reclaim_stale."""
        import engine
        with mock.patch.object(engine, "_code_changed", return_value="engine.py"), \
             mock.patch.object(engine.rail, "release") as rel:
            with self.assertRaises(SystemExit):
                engine._code_changed_exit(self._ctx(), "x")
        self.assertEqual(rel.call_args[0][0], "ep-id")
