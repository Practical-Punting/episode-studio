...
# ONE HEAVY RENDER AT A TIME, ACROSS BOTH PRODUCTION LINES ON THIS LAPTOP.
# A VERBATIM copy of the Inspirational Women line's module — the shared contract is
# the lock file path, the atomic O_EXCL create and the .beat sidecar, so the two
# copies must never diverge. `engine/test_render_lock_copy.py` compares them by
# sha256 and goes red the moment they do. DO NOT EDIT THIS FILE LOCALLY.
...
# ── THE FOURTH STATE: BLOCKED ON ANOTHER JOB ON THIS MACHINE ────────────────────
# (30 Aug 2026, written BEFORE the render lock was wired in, and on purpose.)
#
# The watchdog above knows three states — working, waiting on a human, stuck — and
# the board decides between them with one comparison: has this step been running
# longer than its budget. That comparison is honest for as long as "running" and
# "working" mean the same thing.
#
# 🔴 THE RENDER LOCK BREAKS THAT EQUIVALENCE, AND IT WOULD BREAK IT ON THE WORST
# POSSIBLE STEP. A second production line now shares this 8 GB laptop, and
# assembly will block until its render finishes. `mark_step_started` stamps
# `started_at` before the step function is called and nothing ever resets it, so a
# forty-minute wait plus fifteen minutes of real assembly is fifty-five minutes
# against a forty-five minute budget — and the board would paint:
#
#     "⛔ Stuck — nobody is coming … Nothing is waiting on you — no flag is up —
#      so it is not going to finish by itself. Restarting the engine picks it up
#      where it left off and loses nothing."
#
# Every clause of that is FALSE during a lock wait, and the last one invites the
# operator to kill an engine that is about to be handed the lock — or, worse, one
# that is holding it and is mid-ffmpeg. That is the studio's existing "a step that
# was still working was declared timed out and a destructive retry was offered"
# defect, arriving by a brand new road.
#
# ⚠️ AND THE OBVIOUS FIX IS THE WRONG ONE. Setting `assemble_*` to `budget_s: None`
# would silence the alarm — for the whole step, for ever. EP14 sat on
# assemble_passB for THREE AND A HALF DAYS behind a healthy heartbeat, which is the
# entire reason this budget exists. So the wait is SUBTRACTED, never the budget
# removed: a genuinely hung ffmpeg still trips the alarm on the working time, and a
# legitimate wait never does. Both directions are proved in
# engine/test_waiting_is_not_stuck.py, controls first.
#
# 📌 It records WHO, not just that it is waiting (CLAUDE.md #3: "say who it is
# waiting on"). "Waiting" with no name reads as the machine waiting on itself.
def note_step_waiting(ctx, who: str, waited_s: float) -> None:
    """This step is BLOCKED ON SOMETHING ELSE — not working, and not stuck.

    🔴 IT NEVER TOUCHES `started_at`, for `set_step_label`'s reason: resetting the
    clock would make a genuinely wedged step invisible for ever. The outer clock
    keeps running; the board subtracts the wait from it.
    """
    cur = ctx.state.get("current")
    if not cur:
        return                          # not in a step: nothing to mark
    cur["waiting_on"] = who
    cur["waited_s"] = int(max(0.0, waited_s))
    ctx.save()


def clear_step_waiting(ctx) -> None:
    """The wait is over and the step is working again.

    🔴 `waited_s` IS DELIBERATELY KEPT. It is the accumulated wait, and the board
    subtracts it for the REST of the step — drop it here and the alarm snaps back
    the instant the lock is granted, which is precisely the moment the step starts
    doing the work the budget was measured for.
    """
    cur = ctx.state.get("current")
    if not cur or "waiting_on" not in cur:
        return
    cur.pop("waiting_on", None)
    ctx.save()
...
# ── ONE HEAVY RENDER AT A TIME, ACROSS BOTH PRODUCTION LINES ────────────────────
#
# This 8 GB laptop runs two studios. Their front halves do not compete — painting,
# downloading and narrating are network- and API-bound — but two ffmpeg jobs do not
# run at half speed on 8 GB: they page to disk and one of them eventually dies
# MID-WRITE. So exactly the CPU-bound half is serialised, and nothing else.
#
# ⚠️ THE `with` GOES HERE AND NOT INSIDE `RealProvider.run()`, AND THAT PLACEMENT IS
# THE WHOLE POINT. `run()` is `subprocess.run(..., timeout=PASS_TIMEOUT)` — 2400s —
# and that clock starts when the child starts. Take the lock inside it and a
# forty-minute wait for the other line eats two thirds of the ffmpeg budget, the
# child gets killed for being slow, and `run_step` hands the operator a raw
# `TimeoutExpired` with the entire ffmpeg command line in it. Out here the wait
# happens before any child exists, so PASS_TIMEOUT still measures ffmpeg alone.
#
# 📌 What is NOT wrapped, on purpose: the HeyGen fetch, the SRT alignment, the
# Higgsfield generations and the e-book PDF. They are network- and API-bound and
# keeping them outside is what lets the two lines actually overlap.
WAITING_ON_OTHER_LINE = "the other production line's render"


@contextmanager
def render_lock_held(ctx, what: str):
    """Hold the cross-project render lock, and tell the BOARD we are waiting.

    The lock's own `say` hook is the only place that knows a wait is happening, so
    it is wired to `note_step_waiting` — otherwise the board judges a blocked step
    against a budget measured for a working one and calls a healthy episode stuck.
    See `note_step_waiting` for what that would have printed.

    🔴 IT NEVER STEALS, CLEARS OR FORCES THE LOCK, and it must stay that way: a
    liveness signal is allowed to be wrong here for exactly one reason — nothing
    automatic acts on it. A wrong guess starts a second encoder on one file.
    """
    t0 = time.monotonic()
    # A mock run must EXERCISE this path — it is the spine — but it must never make
    # a real render on the other line wait behind a simulation. Its own lock file.
    path = (str(Path(tempfile.gettempdir()) / "pp-mock-render.lock")
            if ctx.mock else None)

    # ⚠️ EVERY MARKER UPDATE HERE IS BOOKKEEPING, AND BOOKKEEPING MAY NOT FAIL A
    # BUILD. `note_step_waiting`/`clear_step_waiting` end in a rail write, and the
    # `say` one runs inside `acquire`'s wait loop — so a network blip forty minutes
    # into a legitimate wait would come out of `hold.__enter__` and kill a step that
    # is doing nothing wrong. Same reasoning the module applies to its own
    # heartbeat: *"a missed beat is not worth an abort"*. The cost of losing one is
    # that the board's wait figure is a minute stale.
    def _tell_the_board(fn, *a):
        try:
            fn(ctx, *a)
        except Exception as e:                                        # noqa: BLE001
            log(f"   (could not update the board's wait marker: {e})")

    def say(msg, **_kw):
        log(f"   {msg}")
        _tell_the_board(note_step_waiting, WAITING_ON_OTHER_LINE,
                        time.monotonic() - t0)

    try:
        with render_lock.hold("PP", what, path=path, say=say):
            # the wait is over; the work has begun. `waited_s` stays behind.
            _tell_the_board(clear_step_waiting)
            yield
    except render_lock.LockStale:
        # THE MODULE'S OWN MESSAGE NAMES A FILE PATH AND A SHELL COMMAND
        # ("python render_lock.py release --force"). Correct for an engineer, and
        # banned from the operator's box by docs/PP-operator-box-rule.md — Jodie
        # must never be asked to run that. Read the holder through the API rather
        # than parsing the module's prose for the number.
        holder = render_lock.read_holder(path) or {}
        quiet = int((holder.get("age_s") or 0) // 60)
        log(f"   the render lock is held by {holder.get('project')!r} "
            f"({holder.get('job')!r}, pid {holder.get('pid')}) and has not breathed "
            f"for {quiet} min — NOT stealing it; flagging for a human.")
        raise EngineFlag(
            "I'm ready to put this episode together, but the other production line "
            "on this laptop still has the machine and it has stopped responding — "
            f"no sign of life from it for about {quiet} minutes.\n\n"
            "Only one of the two can build a video at a time. If I started anyway "
            "they would both run out of memory and one of the videos would be "
            "damaged halfway through, so I stopped instead.\n\n"
            "Nothing has been lost and nothing has been spent — this episode is "
            "exactly where it was.\n\n"
            "THIS IS A FAULT IN THE STUDIO, NOT SOMETHING TO FIX ON THE BOARD. "
            "Please tell whoever looks after the engine that the other production "
            "line's render has stopped. Once they have sorted it, clear this flag "
            "and I'll pick this straight back up.")
    except render_lock.LockBusy:
        # Only reachable if somebody gives this a timeout. It has none today, and a
        # legitimate wait is not a fault — but if that ever changes, the operator
        # must not meet the raw message either.
        holder = render_lock.read_holder(path) or {}
        log(f"   gave up waiting for the render lock held by {holder.get('project')!r}")
        raise EngineFlag(
            "The other production line on this laptop is still building a video and "
            "I waited as long as I'm allowed to. Only one of us can build at a time.\n\n"
            "Nothing has been lost and nothing has been spent. Clear this flag once "
            "the other one has finished and I'll carry on from here.")
    finally:
        # never let a bookkeeping error MASK the real exception on its way out
        _tell_the_board(clear_step_waiting)


def step_assemble_passA(ctx):
    with render_lock_held(ctx, f"{ep_folder(ctx.ep)} assembly (pass A)"):
        return {"passA": ctx.provider.assemble_passA(ctx.ep)}


def step_assemble_passB(ctx):
    with render_lock_held(ctx, f"{ep_folder(ctx.ep)} assembly (pass B)"):
        out = ctx.provider.assemble_passB(ctx.ep)
    # THE BOOKKEEPING IS OUTSIDE THE LOCK. Give the machine back the moment ffmpeg
    # is done rather than holding it across a rail write.
